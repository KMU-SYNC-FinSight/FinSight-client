...
#### 상태바 틈은 색을 맞춰서 없앤다 (콘텐츠를 밀어 넣지 않는다)

`theme-color` 를 정적으로 하나만 두면 화면마다 상단 색이 달라 띠가 생긴다
(스플래시는 노란 화면인데 상태바만 흰색으로 남는 식).
`src/hooks/useStatusBarColor.ts` 가 라우트가 바뀔 때마다 meta 를 갈아 끼운다.
...
**`apple-mobile-web-app-status-bar-style` 을 `black-translucent` 로 바꾸지 말 것.**
콘텐츠가 상태바 아래까지 올라가서 "edge-to-edge" 가 되는 건 맞지만,
**상태바 글자가 흰색으로 고정**된다. 이 앱은 상단이 흰색이라 시계·배터리가 사라진 것처럼 보이고,
노란 스플래시 위에서도 흰 글씨는 1.70:1 로 안 읽힌다.
색만 맞춰도 틈은 사라지므로 가독성을 버릴 이유가 없다.

같은 이유로 안드로이드 `display: fullscreen` 도 쓰지 않는다 — 상태바가 통째로 사라진다.

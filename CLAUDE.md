...
- `AppFrame` 이 모든 화면을 `max-width: 448px` 로 묶고 가운데 정렬한다.
- 480px 이상에서만 프레임 그림자를 준다 (모바일에서는 화면 전체이므로 불필요).
- 높이는 `100dvh` (모바일 브라우저 주소창 때문에 `100vh` 는 쓰지 않는다).
- 하단 탭바·고정 CTA 는 `env(safe-area-inset-bottom)` 을 더해 홈 인디케이터를 피한다.
- 입력 필드 `font-size` 는 **16px 이상** — 미만이면 iOS 에서 포커스 시 화면이 확대된다.
